...
async function Karyera({ params }) {
  const locale = params?.locale || 'az';
  const internsData = await getInterns(locale)
  const carrierPageData = await getCarrierPage(locale)
  const vacanciesData = await getVacancies(locale)
  const carrierBottomData = await getCarrierBottom(locale)
  ...
  return (
    <div className={karyeraStyle.karyeraMain}>
      ...
      <VacancySearch locale={locale} vacanciesData={vacanciesData} />
      ...
      <div className={karyeraStyle.vacancies}>
        <h3 className={karyeraStyle.vacancyTitle}>{locale==='az' ? "Vakansiyalar" : "Vacancies"}</h3>
        {
          vacanciesData.filter((vacancyActive)=>vacancyActive.is_active===true).map((vacancy)=>{
            return(
              <AboutVacancyCard locale={locale} key={vacancy.id} link={`karyera/${vacancy.id}`} vacancy={vacancy.title} jobInfoTextAbout={vacancy.text.slice(0,207) + '...'}/>
            )
          })
        }
      </div>
